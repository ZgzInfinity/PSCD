...
void controlGasolinera::iniciarRepostaje(int vehiculo, int& elSurtidorLibre, int vecesRepostado, int duracion, ofstream& f){
	...
	while (surtidoresEnUso>=MAX_SURTIDORES || filaCoches.front()!=vehiculo || enRevision){
		hanEsperado++;
...
void controlGasolinera::finalizarRepostaje(int vehiculo, int elSurtidorLibre, ofstream& f){
	...
	if (surtidoresEnUso==0 && deseoDeRevisar){
		revision.notify_all();
	}
	else{
...
// gestión del inicio de revisión
void controlGasolinera::iniciarRevision(int aleatorio, ofstream& f){
	unique_lock<mutex> lck (mtx);
	deseoDeRevisar=true;
	cout << endl << endl;
	f << endl << endl;
	/* Control de la operacion bloqueante 
	 * Solo se puede revisar si los surtidores estan vacios */
	while (surtidoresEnUso>0){
		// Se bloquea la revision
		revision.wait(lck);
	}
	
	// Se modifica el estado de la revision
	enRevision=true;
	deseoDeRevisar=false;
	cout << endl;
	cout << " **** EN REVISION **** " << endl << endl;
	f << " **** EN REVISION **** " << endl << endl;
	for (int i = 0; i < MAX_SURTIDORES; i++){
			// Muestreo del estado de los surtidores
			if (surtidoresLibres[i]){
				cout << " Surtidor " << i << " --> vacio " << endl;
				f << " Surtidor " << i << " --> vacio " << endl;
			}
			cout << endl;
			f << endl;
		}
	totalRevisiones++;
	cout << " Se han realizado " << totalRevisiones << " revisiones " << endl;
	f << " Se han realizado " << totalRevisiones << " revisiones " << endl;
}

//gestión del final de una revisión
void controlGasolinera::finalizarRevision(ofstream& f){
	unique_lock<mutex> lck (mtx);
	cout << endl << endl;
	f << endl << endl;
	cout << " *** TERMINA UNA REVISION  *** "  << endl;
	f << " *** TERMINA UNA REVISION *** "  << endl;
	cout << endl << endl;
	f << endl << endl;
	...
	/* Se avisa a los coches de que pueden repostar
	 * pero se avisa 4 veces para que entren 4 coches seguidos */
	repostaje.notify_all();
	repostaje.notify_all();
	repostaje.notify_all();
	repostaje.notify_all();
